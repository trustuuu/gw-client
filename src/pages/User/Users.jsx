...
function Users({
  ...
  const handleChangeCheck = function (e) {
    ...
    if (isChecked) {
      parentCallback([...checkedItems, ...users.filter((g) => g.id === item)]);
      setCheckedItems([...checkedItems, ...users.filter((g) => g.id === item)]);
    } else {
                ...
                >
                  <div className="self-center mr-3">
                    <Checkbox
                      ...
                      id={user.id}
                      checked={
                        checkedItems.filter((i) => i.id === user.id).length > 0
                      }
                  ...
                  <div className="grow flex items-center text-sm py-2">
                    <div
                      className="grow flex"
                      onClick={
                        noDetailView ? null : onClickView.bind(this, user)
                      }
                    >
                      <div className="self-center uppercase w-1/6 min-w-48">
