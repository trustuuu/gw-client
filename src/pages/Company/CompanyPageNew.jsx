...
function CompanyPage() {
  ...
  const { data: companies = [], isLoading } = useQuery({
    ...
    enabled: !!company?.id, // ID가 있을 때만 실행
    onError: (error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (ids) => companyApi.remove(ids),
    onSuccess: () => {
      console.log("delete success");
      queryClient.invalidateQueries(["companies", company?.id]); // 캐시 무효화 및 자동 새로고침
      setCheckedItems([]);
    },
    onError: (err) => {
  ...
  // 핸들러 함수들
  const paginate = (pageNumber, startPage) => {
  ...
  const onClickNew = () => navigate("/onboarding-company-new", { state: { mode: "new", company, parent: company } });
  const onClickView = (item) => navigate("/onboarding-company-new", { state: { company: item, mode: "view", parent: company } });
  const onClickEdit = (item) => navigate("/onboarding-company-new", { state: { company: item, mode: "edit", parent: company } });
