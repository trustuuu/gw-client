...
function Applications({
  ...
  const handleChangeCheck = function (e) {
    ...
    if (isChecked) {
      parentCallback([
        ...checkedItems,
        ...applications.filter((i) => i.client_id == item),
      ]);
      setCheckedItems([
        ...checkedItems,
        ...applications.filter((i) => i.client_id == item),
      ...
      parentCallback(checkedItems.filter((i) => i.client_id != item));
      setCheckedItems(checkedItems.filter((i) => i.client_id != item));
    }
  };
                ...
                >
                  <div className="self-center mr-3">
                    <Checkbox
                      ...
                      checked={
                        checkedItems.filter(
                          (i) => i.client_id == application.client_id
                        ).length > 0
                      }
                  ...
                  <div className="grow flex items-center border-b border-slate-100 dark:border-slate-700 text-sm py-2">
                    <div
                      className="grow flex"
                      onClick={onClickView.bind(this, application)}
                    >
                      <div className="self-center  w-1/6 min-w-48">
                        {application ? application.client_name : ""}
                      </div>
                      <div className="text-left justify-self-start w-4/6">
                        {application ? application.audience : ""}
                      </div>
                    </div>
                    <div
                      className="shrink-0 self-end ml-2 w-1/6"
                      onClick={onClickEdit.bind(this, application)}
                    >
