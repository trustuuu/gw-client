...
function ApiSystemUsersGroupsPage({ api: apiState }) {
  ...
  const onClickDel = async function () {
    setIsLoading(true);
    await systemApi.removeUsersAndGroups(
      company.id,
      domain.id,
      api.id,
      checkedItems,
  ...
  const onClickAdd = async function () {
    ...
    await systemApi.createUsersAndGroups(
      company.id,
      domain.id,
      api.id,
      totalItems.map((item) => {
...
export default ApiSystemUsersGroupsPage;
